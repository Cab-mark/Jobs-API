...
def build_job_payload(external_id: str = "ext-1") -> dict:
    ...
    payload = {
        ...
        "applyDetail": "Send CV",
        "closingDate": (datetime.now(timezone.utc) + timedelta(days=7)).isoformat(),
...
def test_create_job_and_get_summary_and_detail():
    ...
    assert detail["closingDate"].startswith(payload["closingDate"][:10])
